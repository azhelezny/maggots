-- SYNOMINS
CREATE SYNONYM SY_CUSTOM_2 FOR INC_BACKUP_SCHEMA.SYN_TABLE;
CREATE SYNONYM SY_DEFAULT_2 FOR SYN_TABLE;
DROP SYNONYM SY_CUSTOM_0;
DROP SYNONYM SY_DEFAULT_0;

-- SEQUENCES
CREATE SEQUENCE SE_2 START WITH 200;
DROP SEQUENCE SE_0 RESTRICT;

-- USERS AND ROLES

CALL SYSCS_UTIL.SYSCS_CREATE_USER('user_1', 'user_1');
CREATE ROLE role_1;
GRANT SELECT ON TABLE INC_BACKUP_SCHEMA.TUSERS_1 TO user_1;
GRANT SELECT ON TABLE TROLES_1 TO role_1;
GRANT role_1 to user_1;

GRANT SELECT ON TABLE INC_BACKUP_SCHEMA.TUSERS_1 TO user_0;
GRANT SELECT ON TABLE TROLES_1 TO role_0;
GRANT role_1 to user_0;

REVOKE SELECT ON TABLE INC_BACKUP_SCHEMA.TUSERS_0 FROM user_4;
REVOKE SELECT ON TABLE TROLES_0 FROM role_4;
REVOKE role_0 from user_4;
...
SELECT count(*) as INDEXES from sys.SYSCONGLOMERATES where isindex=true and CONGLOMERATENAME in ('I_CUSTOM_0', 'I_DEFAULT_0') and schemaid in (SELECT SCHEMAID from sys.SYSSCHEMAS WHERE SCHEMANAME in ('INC_BACKUP_SCHEMA', 'SPLICE'));
-- expected 0

-- TRIGGERS/TABLES/VIEWS

DROP TRIGGER INC_BACKUP_SCHEMA.T_CUSTOM_0;
DROP TRIGGER T_DEFAULT_0;
...
-- EXTERNAL TABLE
INSERT INTO EXT_ORC_0 values (4);
...
CREATE TABLE INC_BACKUP_SCHEMA.TABLE_1 (pk_col CHAR constraint pk_values_1 primary key, nfk_col CHAR constraint fk_values_1 references INC_BACKUP_SCHEMA.fk_table(c), ufk_col CHAR, foreign key (ufk_col) references INC_BACKUP_SCHEMA.fk_table(c), nun_col CHAR constraint un_values_1 unique, uun_col CHAR unique, inx_col CHAR, ch_col CHAR (8), lv_col CHAR (8), vc_col VARCHAR (8), dt_col DATE, ti_col TIME, ts_col TIMESTAMP, bl_col BLOB (8), cl_col CLOB (8), bi_col BIGINT, dc_col DECIMAL (5,2), do_col DOUBLE, fl_col FLOAT (24), in_col INTEGER, nu_col NUMERIC (5,2), re_col REAL, si_col SMALLINT, bo_col BOOLEAN);
...
CREATE TABLE TABLE_1 (pk_col CHAR constraint pk_values_1 primary key, nfk_col CHAR constraint fk_values_1 references INC_BACKUP_SCHEMA.fk_table(c), ufk_col CHAR, foreign key (ufk_col) references INC_BACKUP_SCHEMA.fk_table(c), nun_col CHAR constraint un_values_1 unique, uun_col CHAR unique, inx_col CHAR, ch_col CHAR (8), lv_col CHAR (8), vc_col VARCHAR (8), dt_col DATE, ti_col TIME, ts_col TIMESTAMP, bl_col BLOB (8), cl_col CLOB (8), bi_col BIGINT, dc_col DECIMAL (5,2), do_col DOUBLE, fl_col FLOAT (24), in_col INTEGER, nu_col NUMERIC (5,2), re_col REAL, si_col SMALLINT, bo_col BOOLEAN);
...
CREATE INDEX INC_BACKUP_SCHEMA.I_CUSTOM_1 ON INC_BACKUP_SCHEMA.TABLE_1(inx_col);
CREATE INDEX I_DEFAULT_1 ON TABLE_1(inx_col);

-- VIEWS

CREATE VIEW INC_BACKUP_SCHEMA.VIEW_1 as select * from INC_BACKUP_SCHEMA.TABLE_1 order by nun_col;
CREATE VIEW VIEW_1 as select * from TABLE_1 order by nun_col;

-- TRIGGERS

CREATE TRIGGER INC_BACKUP_SCHEMA.T_CUSTOM_1 AFTER INSERT ON INC_BACKUP_SCHEMA.TABLE_1
FOR EACH ROW
INSERT INTO INC_BACKUP_SCHEMA.audit_triggers values ('T_CUSTOM_1');

CREATE TRIGGER T_DEFAULT_1 AFTER INSERT ON TABLE_1
FOR EACH ROW
INSERT INTO INC_BACKUP_SCHEMA.audit_triggers values ('T_DEFAULT_1');

-- FUNCTIONS

CREATE FUNCTION INC_BACKUP_SCHEMA.GET_ABS_1(NUM INTEGER)
RETURNS INTEGER
PARAMETER STYLE JAVA
NO SQL LANGUAGE JAVA
EXTERNAL NAME 'java.lang.Math.abs';

CREATE FUNCTION GET_ABS_1(NUM INTEGER)
RETURNS INTEGER
PARAMETER STYLE JAVA
NO SQL LANGUAGE JAVA
EXTERNAL NAME 'java.lang.Math.abs';
...
CREATE FUNCTION READ_CUSTOM_MAIN_TYPES_1()
...
-- to check SELECT * FROM TABLE (READ_CUSTOM_MAIN_TYPES_1()) b order by it_c, vc_c, db_c, dc_c, dt_c desc;

/*Disabled due to DB-5715
--EXTERNAL TABLE

CREATE EXTERNAL TABLE EXT_ORC_1 (id INT) STORED AS ORC location '/tmp/testdb/sandbox/1';
INSERT INTO EXT_ORC_1 values (1),(2),(3);
*/
