...
SELECT count(*) from sys.SYSSEQUENCES where sequencename in ('SE_1', 'SE_2');
-- ecpected 2
SELECT count(*) from sys.SYSSEQUENCES where sequencename in ('SE_0', 'SE_3', 'SE_4', 'SE_5');
-- ecpected 0
...
SELECT count(USERNAME) from sys.SYSUSERS where USERNAME in ('USER_0', 'USER_1', 'USER_4');
-- expected 3
SELECT count(USERNAME) from sys.SYSUSERS where USERNAME in ('USER_2', 'USER_3');
-- expected 0

SELECT count(*) from sys.SYSROLES where ROLEID in ('ROLE_0', 'ROLE_1' , 'ROLE_4') and GRANTEE != 'SPLICE';
-- expected 4
SELECT count(*) from sys.SYSROLES where ROLEID in ('ROLE_2', 'ROLE_3');
-- expected 0
...
--expected ERROR 42502: User 'USER_1' does not have SELECT permission on column 'I' of table 'INC_BACKUP_SCHEMA'.'TUSERS_0'.
SELECT * FROM INC_BACKUP_SCHEMA.TUSERS_2;
--expected ERROR 42502: User 'USER_1' does not have SELECT permission on column 'I' of table 'INC_BACKUP_SCHEMA'.'TUSERS_2'.
SELECT * FROM INC_BACKUP_SCHEMA.TUSERS_3;
--expected ERROR 42502: User 'USER_1' does not have SELECT permission on column 'I' of table 'INC_BACKUP_SCHEMA'.'TUSERS_3'.
SELECT * FROM INC_BACKUP_SCHEMA.TUSERS_4;
--expected ERROR 42502: User 'USER_1' does not have SELECT permission on column 'I' of table 'INC_BACKUP_SCHEMA'.'TUSERS_4'.
...
--expected ERROR 42502: User 'USER_1' does not have SELECT permission on column 'I' of table 'SPLICE'.'TROLES_0'.
SELECT * FROM SPLICE.TROLES_2;
--expected ERROR 42502: User 'USER_1' does not have SELECT permission on column 'I' of table 'SPLICE'.'TROLES_2'.
SELECT * FROM SPLICE.TROLES_3;
--expected ERROR 42502: User 'USER_1' does not have SELECT permission on column 'I' of table 'SPLICE'.'TROLES_3'.
SELECT * FROM SPLICE.TROLES_4;
--expected ERROR 42502: User 'USER_1' does not have SELECT permission on column 'I' of table 'SPLICE'.'TROLES_4'.
...
SET ROLE role_1;
--expected ERROR 0P000: Invalid role specification, role not granted to current user or PUBLIC: 'ROLE_1'.
SET ROLE role_2;
...
-- 2 expected
SELECT count(*) as TABLES from sys.SYSTABLES where tablename IN('TABLE_2', 'TABLE_3', 'TABLE_4') and schemaid in (SELECT SCHEMAID from sys.SYSSCHEMAS WHERE SCHEMANAME IN ('INC_BACKUP_SCHEMA', 'SPLICE'));
-- 0 expected
...
-- 2 expected
SELECT count(*) as VIEWS from sys.SYSTABLES where tablename IN('VIEW_2', 'VIEW_3', 'VIEW_4') and schemaid in (SELECT SCHEMAID from sys.SYSSCHEMAS WHERE SCHEMANAME IN ('INC_BACKUP_SCHEMA', 'SPLICE'));
-- 0 expected
...
SELECT count(*) as INDEXES from sys.SYSCONGLOMERATES where isindex=true and CONGLOMERATENAME in ('I_CUSTOM_1', 'I_DEFAULT_1') in (SELECT tableid from sys.SYSTABLES where tablename = 'TABLE_0') and schemaid in (SELECT SCHEMAID from sys.SYSSCHEMAS WHERE SCHEMANAME in ('INC_BACKUP_SCHEMA', 'SPLICE'));
-- 2 expected
SELECT count(*) as INDEXES from sys.SYSCONGLOMERATES where isindex=true and CONGLOMERATENAME in ('I_CUSTOM_0', 'I_CUSTOM_2', 'I_CUSTOM_3', 'I_CUSTOM_4', 'I_DEFAULT_0', 'I_DEFAULT_2', 'I_DEFAULT_3', 'I_DEFAULT_4') in (SELECT tableid from sys.SYSTABLES where tablename = 'TABLE_0') and schemaid in (SELECT SCHEMAID from sys.SYSSCHEMAS WHERE SCHEMANAME in ('INC_BACKUP_SCHEMA', 'SPLICE'));
-- 0 expected
...
SELECT triggername from sys.SYSTRIGGERS where tableid in (SELECT tableid from sys.SYSTABLES where tablename = 'TABLE_1') and schemaid in (SELECT SCHEMAID from sys.SYSSCHEMAS WHERE SCHEMANAME in ('INC_BACKUP_SCHEMA', 'SPLICE'));
-- expected 
		--T_CUSTOM_1
		--T_DEFAULT_1

INSERT INTO INC_BACKUP_SCHEMA.TABLE_1(pk_col, nfk_col, ufk_col, nun_col, uun_col, inx_col) VALUES ('T', 'c', 'c', '5', '5', ')');
INSERT INTO TABLE_1(pk_col, nfk_col, ufk_col, nun_col, uun_col, inx_col) VALUES ('T', 'c', 'c', '5', '5', ')');
...
DELETE FROM INC_BACKUP_SCHEMA.TABLE_1 WHERE pk_col ='T';
DELETE FROM TABLE_1 WHERE pk_col ='T';
...
-- expected 2
SELECT count(*) from sys.SYSALIASES where alias in ('GET_ABS_0', 'GET_ABS_2', 'GET_ABS_3', 'GET_ABS_4') and schemaid in (SELECT SCHEMAID from sys.SYSSCHEMAS WHERE SCHEMANAME in ('INC_BACKUP_SCHEMA', 'SPLICE'));
-- expected 0
