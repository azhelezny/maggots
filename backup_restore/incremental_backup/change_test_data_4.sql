-- SYNOMINS
CREATE SYNONYM SY_CUSTOM_5 FOR INC_BACKUP_SCHEMA.SYN_TABLE;
CREATE SYNONYM SY_DEFAULT_5 FOR SYN_TABLE;
DROP SYNONYM SY_CUSTOM_4;
DROP SYNONYM SY_DEFAULT_4;

-- SEQUENCES
CREATE SEQUENCE SE_5 START WITH 500;
...
GRANT SELECT ON TABLE INC_BACKUP_SCHEMA.TUSERS_4 TO user_0;
GRANT SELECT ON TABLE TROLES_4 TO role_0;
GRANT role_4 to user_0;

REVOKE SELECT ON TABLE INC_BACKUP_SCHEMA.TUSERS_3 FROM user_4;
REVOKE SELECT ON TABLE TROLES_3 FROM role_4;
...
SELECT count(*) as INDEXES from sys.SYSCONGLOMERATES where isindex=true and CONGLOMERATENAME in ('I_CUSTOM_3', 'I_DEFAULT_3') and schemaid in (SELECT SCHEMAID from sys.SYSSCHEMAS WHERE SCHEMANAME in ('INC_BACKUP_SCHEMA', 'SPLICE'));
-- expected 0

-- TRIGGERS/TABLES/VIEWS

DROP TRIGGER INC_BACKUP_SCHEMA.T_CUSTOM_3;
DROP TRIGGER T_DEFAULT_3;
...
-- EXTERNAL TABLE
INSERT INTO EXT_ORC_0 values (7);
INSERT INTO EXT_ORC_1 values (6);
INSERT INTO EXT_ORC_2 values (5);
INSERT INTO EXT_ORC_3 values (4);
...
CREATE TABLE INC_BACKUP_SCHEMA.TABLE_4 (pk_col CHAR constraint pk_values_4 primary key, nfk_col CHAR constraint fk_values_4 references INC_BACKUP_SCHEMA.fk_table(c), ufk_col CHAR, foreign key (ufk_col) references INC_BACKUP_SCHEMA.fk_table(c), nun_col CHAR constraint un_values_4 unique, uun_col CHAR unique, inx_col CHAR, ch_col CHAR (8), lv_col CHAR (8), vc_col VARCHAR (8), dt_col DATE, ti_col TIME, ts_col TIMESTAMP, bl_col BLOB (8), cl_col CLOB (8), bi_col BIGINT, dc_col DECIMAL (5,2), do_col DOUBLE, fl_col FLOAT (24), in_col INTEGER, nu_col NUMERIC (5,2), re_col REAL, si_col SMALLINT, bo_col BOOLEAN);
INSERT INTO INC_BACKUP_SCHEMA.TABLE_4(pk_col, nfk_col, ufk_col, nun_col, uun_col, inx_col, ch_col, lv_col, vc_col, dt_col, ti_col, ts_col, bl_col, cl_col, bi_col, dc_col, do_col, fl_col, in_col, nu_col, re_col, si_col, bo_col) VALUES ('{', 'a', 'a', '1', '1', '(', 'W+ ?vD:W', 'jYj)mz#f', 'mKJ}G ,;', '4527-03-22', '20:32:58', '1970-01-22 14:33:48.788', CAST(X'62' AS BLOB), '40', 458263475691428698, 58.05, 0.8541313654300117, 0.46841645, -14957700, 65.96, 0.27229637, 11479, true);
...
CREATE INDEX INC_BACKUP_SCHEMA.I_CUSTOM_4 ON INC_BACKUP_SCHEMA.TABLE_4(inx_col);
CREATE INDEX I_DEFAULT_4 ON TABLE_4(inx_col);

-- VIEWS

CREATE VIEW INC_BACKUP_SCHEMA.VIEW_4 as select * from INC_BACKUP_SCHEMA.TABLE_4 order by nun_col;
CREATE VIEW VIEW_4 as select * from TABLE_4 order by nun_col;

-- TRIGGERS

CREATE TRIGGER INC_BACKUP_SCHEMA.T_CUSTOM_4 AFTER INSERT ON INC_BACKUP_SCHEMA.TABLE_4
FOR EACH ROW
INSERT INTO INC_BACKUP_SCHEMA.audit_triggers values ('T_CUSTOM_4');

CREATE TRIGGER T_DEFAULT_4 AFTER INSERT ON TABLE_4
FOR EACH ROW
INSERT INTO INC_BACKUP_SCHEMA.audit_triggers values ('T_DEFAULT_4');

-- FUNCTIONS

CREATE FUNCTION INC_BACKUP_SCHEMA.GET_ABS_4(NUM INTEGER)
RETURNS INTEGER
PARAMETER STYLE JAVA
NO SQL LANGUAGE JAVA
EXTERNAL NAME 'java.lang.Math.abs';

CREATE FUNCTION GET_ABS_4(NUM INTEGER)
RETURNS INTEGER
PARAMETER STYLE JAVA
NO SQL LANGUAGE JAVA
EXTERNAL NAME 'java.lang.Math.abs';
...
CREATE FUNCTION READ_CUSTOM_MAIN_TYPES_4()
...
-- to check SELECT * FROM TABLE (READ_CUSTOM_MAIN_TYPES_4()) b order by it_c, vc_c, db_c, dc_c, dt_c desc;

/*Disabled due to DB-5715
--EXTERNAL TABLE

CREATE EXTERNAL TABLE EXT_ORC_4 (id INT) STORED AS ORC location '/tmp/testdb/sandbox/4';
INSERT INTO EXT_ORC_4 values (1),(2),(3);
*/
;