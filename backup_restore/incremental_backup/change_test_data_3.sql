-- SYNOMINS
CREATE SYNONYM SY_CUSTOM_4 FOR INC_BACKUP_SCHEMA.SYN_TABLE;
CREATE SYNONYM SY_DEFAULT_4 FOR SYN_TABLE;
DROP SYNONYM SY_CUSTOM_3;
DROP SYNONYM SY_DEFAULT_3;

-- SEQUENCES
CREATE SEQUENCE SE_4 START WITH 400;
...
DROP ROLE role_2;
CALL SYSCS_UTIL.SYSCS_CREATE_USER('user_3', 'user_3');
CREATE ROLE role_3;
GRANT SELECT ON TABLE INC_BACKUP_SCHEMA.TUSERS_3 TO user_3;
GRANT SELECT ON TABLE TROLES_3 TO role_3;
GRANT role_3 to user_3;

GRANT SELECT ON TABLE INC_BACKUP_SCHEMA.TUSERS_3 TO user_0;
GRANT SELECT ON TABLE TROLES_3 TO role_0;
GRANT role_3 to user_0;

REVOKE SELECT ON TABLE INC_BACKUP_SCHEMA.TUSERS_2 FROM user_4;
REVOKE SELECT ON TABLE TROLES_2 FROM role_4;
...
SELECT count(*) as INDEXES from sys.SYSCONGLOMERATES where isindex=true and CONGLOMERATENAME in ('I_CUSTOM_2', 'I_DEFAULT_2') and schemaid in (SELECT SCHEMAID from sys.SYSSCHEMAS WHERE SCHEMANAME in ('INC_BACKUP_SCHEMA', 'SPLICE'));
-- expected 0

-- TRIGGERS/TABLES/VIEWS

DROP TRIGGER INC_BACKUP_SCHEMA.T_CUSTOM_2;
DROP TRIGGER T_DEFAULT_2;
...
-- EXTERNAL TABLE
INSERT INTO EXT_ORC_0 values (6);
INSERT INTO EXT_ORC_1 values (5);
INSERT INTO EXT_ORC_2 values (4);
...
CREATE TABLE INC_BACKUP_SCHEMA.TABLE_3 (pk_col CHAR constraint pk_values_3 primary key, nfk_col CHAR constraint fk_values_3 references INC_BACKUP_SCHEMA.fk_table(c), ufk_col CHAR, foreign key (ufk_col) references INC_BACKUP_SCHEMA.fk_table(c), nun_col CHAR constraint un_values_3 unique, uun_col CHAR unique, inx_col CHAR, ch_col CHAR (8), lv_col CHAR (8), vc_col VARCHAR (8), dt_col DATE, ti_col TIME, ts_col TIMESTAMP, bl_col BLOB (8), cl_col CLOB (8), bi_col BIGINT, dc_col DECIMAL (5,2), do_col DOUBLE, fl_col FLOAT (24), in_col INTEGER, nu_col NUMERIC (5,2), re_col REAL, si_col SMALLINT, bo_col BOOLEAN);
...
CREATE TABLE TABLE_3 (pk_col CHAR constraint pk_values_3 primary key, nfk_col CHAR constraint fk_values_3 references INC_BACKUP_SCHEMA.fk_table(c), ufk_col CHAR, foreign key (ufk_col) references INC_BACKUP_SCHEMA.fk_table(c), nun_col CHAR constraint un_values_3 unique, uun_col CHAR unique, inx_col CHAR, ch_col CHAR (8), lv_col CHAR (8), vc_col VARCHAR (8), dt_col DATE, ti_col TIME, ts_col TIMESTAMP, bl_col BLOB (8), cl_col CLOB (8), bi_col BIGINT, dc_col DECIMAL (5,2), do_col DOUBLE, fl_col FLOAT (24), in_col INTEGER, nu_col NUMERIC (5,2), re_col REAL, si_col SMALLINT, bo_col BOOLEAN);
...
CREATE INDEX INC_BACKUP_SCHEMA.I_CUSTOM_3 ON INC_BACKUP_SCHEMA.TABLE_3(inx_col);
CREATE INDEX I_DEFAULT_3 ON TABLE_3(inx_col);

-- VIEWS

CREATE VIEW INC_BACKUP_SCHEMA.VIEW_3 as select * from INC_BACKUP_SCHEMA.TABLE_3 order by nun_col;
CREATE VIEW VIEW_3 as select * from TABLE_3 order by nun_col;

-- TRIGGERS

CREATE TRIGGER INC_BACKUP_SCHEMA.T_CUSTOM_3 AFTER INSERT ON INC_BACKUP_SCHEMA.TABLE_3
FOR EACH ROW
INSERT INTO INC_BACKUP_SCHEMA.audit_triggers values ('T_CUSTOM_3');

CREATE TRIGGER T_DEFAULT_3 AFTER INSERT ON TABLE_3
FOR EACH ROW
INSERT INTO INC_BACKUP_SCHEMA.audit_triggers values ('T_DEFAULT_3');

-- FUNCTIONS

CREATE FUNCTION INC_BACKUP_SCHEMA.GET_ABS_3(NUM INTEGER)
RETURNS INTEGER
PARAMETER STYLE JAVA
NO SQL LANGUAGE JAVA
EXTERNAL NAME 'java.lang.Math.abs';

CREATE FUNCTION GET_ABS_3(NUM INTEGER)
RETURNS INTEGER
PARAMETER STYLE JAVA
NO SQL LANGUAGE JAVA
EXTERNAL NAME 'java.lang.Math.abs';
...
CREATE FUNCTION READ_CUSTOM_MAIN_TYPES_3()
...
-- to check SELECT * FROM TABLE (READ_CUSTOM_MAIN_TYPES_3()) b order by it_c, vc_c, db_c, dc_c, dt_c desc;

/*Disabled due to DB-5715
--EXTERNAL TABLE

CREATE EXTERNAL TABLE EXT_ORC_3 (id INT) STORED AS ORC location '/tmp/testdb/sandbox/3';
INSERT INTO EXT_ORC_3 values (1),(2),(3);
*/
